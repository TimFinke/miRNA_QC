...
knitr::include_graphics("Diagram_miRNAs_fix2.png")

```

In addition to the original dataset, 5 datasets have been constructed with microRNA counts, 1 dataset containing covariates inherent to the microRNA sequencing-process, as well as 1 dataset containing descriptives on the two datasets with log-transformed counts. The original (unmodified) data has been archived as well as the raw counts which have undergone QC. The other datasets are available on the Midas-server. Below is a brief desription of each dataset, including reasons why users may or may not want to use these in their analyses: \
